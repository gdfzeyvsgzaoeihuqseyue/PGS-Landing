  ...
  " class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div v-if="randomTestimonies.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div v-for="testimonial in randomTestimonies" :key="testimonial.id" :class="[
        ...
        <div v-if="testimonial.note && testimonial.note >= 1 && testimonial.note <= 5" class="flex items-center mb-4">
          <IconStarFilled v-for="n in testimonial.note" :key="`filled-${n}`" class="h-5 w-5 text-yellow-400" />
          <IconStar v-for="n in 5 - testimonial.note" :key="`empty-${n}`" class="h-5 w-5 text-gray-300" />
        </div>
