<template>
  <div v-if="steps && steps.length > 0" class="my-3 border border-gray-200 rounded-lg overflow-hidden">
    ...
    <Transition name="expand">
      <div v-if="isExpanded" class="p-3 bg-white border-t border-gray-200">
        <div class="space-y-2">
          <div
          ...
          >
            ...
            <div class="flex-1">
              ...
              <div v-if="step.data" class="mt-1 text-xs text-gray-500">
                <pre class="bg-gray-100 p-2 rounded text-xs overflow-x-auto">{{ formatData(step.data) }}</pre>
              </div>
            </div>
          </div>
        </div>
...
import { ref } from 'vue';
import type { ThinkingStep } from '@/types/NoahBot';
