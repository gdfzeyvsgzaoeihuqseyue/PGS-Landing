<template>
  <main v-if="solutionStore.currentSolution">
    <!-- Hero section -->
    <section class="relative py-24 bg-cover bg-center" :style="`background-image: url('${solutionStore.currentSolution?.logoDesk}');`">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-12">
        ...
        <div class="md:col-span-2">
          ...
          <template v-if="solutionStore.currentSolution.features && solutionStore.currentSolution.features.length > 0">
            ...
            <ul class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              <li v-for="(feature, index) in solutionStore.currentSolution.features" :key="index" class="flex items-center">
          ...
          <template v-if="solutionStore.currentSolution.docs && solutionStore.currentSolution.docs.length > 0">
            <h3 class="text-2xl font-bold mb-4">Documentation & Guides</h3>
            <ul class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              <li v-for="(doc, index) in solutionStore.currentSolution.docs" :key="index">
        ...
        <aside class="md:col-span-1">
          <div class="bg-gray-50 p-6 rounded-lg shadow-md">
            <img :src="solutionStore.currentSolution?.logo" :alt="solutionStore.currentSolution?.name" class="h-24 w-auto mx-auto mb-6 object-contain"
            ...
            <a :href="solutionStore.currentSolution?.ctaLink"
              :class="['block text-center py-3 px-6 rounded-md font-medium',
                solutionStore.currentSolution?.disabled ? 'bg-gray-200 text-gray-400 cursor-not-allowed' :
                  'bg-primary text-white hover:bg-secondary']" :aria-disabled="solutionStore.currentSolution?.disabled" target="_blank"
            ...
            <div class="mt-6 pt-6 border-t border-gray-200">
              <ul class="space-y-3">
                ...
                <li><strong>Dernière mise à jour :</strong> {{ formatDate(solutionStore.currentSolution?.updatedAt) }}</li>
              </ul>
            </div>
          </div>
        </aside>
    ...
    <!-- FAQ et TUTORIELS -->
    <section v-if="(solutionStore.currentSolution.faq && solutionStore.currentSolution.faq.length > 0) || (solutionStore.currentSolution.tutorials && solutionStore.currentSolution.tutorials.length > 0)"
      ...
      <template v-if="solutionStore.currentSolution.faq && solutionStore.currentSolution.faq.length > 0">
        <div class="bg-gray-50 p-6 rounded-lg shadow-md mt-8">
          <h3 class="text-xl font-bold mb-4">FAQ sur {{ solutionStore.currentSolution.name }}</h3>
          <div v-if="solutionStore.currentSolution.faq.length > 2" class="mb-4">
      ...
      <template v-if="solutionStore.currentSolution.tutorials && solutionStore.currentSolution.tutorials.length > 0">
        <div class="bg-gray-50 p-6 rounded-lg shadow-md mt-8">
          <h3 class="text-xl font-bold mb-4">Tutoriels Vidéo</h3>
